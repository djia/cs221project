...
% collect the non-noise and non-fixation trials
	...
	for i = 1:nTrials
		...
		PixelsPerImage = length(examples(1,:))/16;
		TrainingExamples = [];
		TestExamples = [];
		for n = 1:16
		    examplesTrainingPerImage = examplesTraining(:,(n-1)*PixelsPerImage + 1 : n*PixelsPerImage);
			examplesTestPerImage = examplesTest(:,(n-1)*PixelsPerImage + 1 : n*PixelsPerImage);
%           [examplesTrainingPerImage, mapping] = compute_mapping(examplesTrainingPerImage, 'PCA', 78);
%		    examplesTestPerImage = out_of_sample(examplesTestPerImage, mapping);
			
			[examplesTrainingPerImage, mapping] = myPCA(examplesTrainingPerImage');
%			size(mapping)
		    examplesTrainingPerImage = examplesTrainingPerImage';
		    examplesTestPerImage = mapping.U'*(examplesTestPerImage' - mapping.mean);
		    examplesTestPerImage = examplesTestPerImage';
			
			TrainingExamples = [TrainingExamples, examplesTrainingPerImage];
			TestExamples = [TestExamples, examplesTestPerImage];
		end
		    	

		examplesTraining = TrainingExamples;
		examplesTest = TestExamples;
